...
// PHPExecutionError wraps both general execution errors and specific PHP error results.
type PHPExecutionError struct {
	ExecErr        error            // General error from frankenphp.ServeHTTP (e.g., context canceled)
	PHPErrorResult *php.ErrorResult // Specific PHP error details (fatal, warning, notice)
	RawOutput      string           // Raw PHP output for detailed inspection
}
