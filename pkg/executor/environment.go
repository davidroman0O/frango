...
func (e *Executor) buildPhpEnvironment(
	...
	if config.DevelopmentMode && config.DisplayErrors {
		// In dev mode, show all errors, and change CWD
		phpEnv["auto_prepend_text"] = "<?php chdir(__DIR__); ini_set('display_errors', '1'); ini_set('display_startup_errors', '1'); error_reporting(E_ALL); ?>"
		phpEnv["html_errors"] = "1"         // Enable HTML formatted errors
		phpEnv["log_errors"] = "1"          // Log errors
		phpEnv["error_log"] = "/dev/stderr" // Log to stderr
	} else {
		// Production mode: Hide errors from output, log them, and change CWD
		phpEnv["auto_prepend_text"] = "<?php chdir(__DIR__); ini_set('display_errors', '0'); ini_set('display_startup_errors', '0'); error_reporting(E_ALL & ~E_DEPRECATED & ~E_STRICT); ?>"
		phpEnv["html_errors"] = "0"
		phpEnv["log_errors"] = "1"
		phpEnv["error_log"] = "/dev/stderr" // Log to stderr is often still useful
	}
...
// setupPhpEnvironment sets up the PHP environment variables
func (e *Executor) setupPhpEnvironment(requestData *RequestData, scriptPath, resolvedPath string, renderFn func(w http.ResponseWriter, r *http.Request) map[string]interface{}, w http.ResponseWriter, r *http.Request) (map[string]string, error) {
	...
	// Set document root to the physical directory containing the script
	documentRoot := filepath.Dir(resolvedPath)
	// Set script name based on the physical script's base name
	originalScriptName := "/" + filepath.Base(resolvedPath) // Use physical basename
	...
	e.addScriptEnvironment(phpEnv, resolvedPath, scriptPath) // Pass both paths
...
func (e *Executor) addScriptEnvironment(phpEnv map[string]string, resolvedPath, scriptPath string) {
	...
	// Set script directory based on physical location for includes if chdir works
	scriptDir := filepath.Dir(resolvedPath)
	phpEnv["SCRIPT_DIR"] = scriptDir // Keep physical for potential includes

	// Add logical filename and dirname for __FILE__ and __DIR__ consistency
	// Revert to physical paths for consistency with physical alignment
	phpEnv["FRANGO_LOGICAL_FILENAME"] = resolvedPath // Use physical path
	phpEnv["FRANGO_LOGICAL_DIR"] = scriptDir         // Use physical path's dir

	// Re-enable chdir for physical path alignment
	phpEnv["FRANGO_DO_CHDIR"] = "1" // Re-enable chdir
}
