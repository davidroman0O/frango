...
// StandardGlobalsProvider provides the default PHP globals script
type StandardGlobalsProvider struct{}

// GetPHPGlobalsScript returns the standard PHP globals script
func (p *StandardGlobalsProvider) GetPHPGlobalsScript() string {
	return phpGlobalsScript
}

// GetPHPGlobalsPath returns the standard path for the PHP globals script
func (p *StandardGlobalsProvider) GetPHPGlobalsPath() string {
...
func UpdateVFS(vfs *vfs.VFS) error {
	...
	if vfs.FileExists(globalsPath) {
		return nil // Already installed
	}

	// Install the globals script
	return vfs.CreateVirtualFile(globalsPath, []byte(provider.GetPHPGlobalsScript()))
}
