 ...
 * 
 * This script displays all form data received via any method
 * It shows both environment variables and standard PHP superglobals
 */

// ./v1/playground/demo/forms/form-test.php
...
// Display Frango custom superglobals
echo "=== FRANGO CUSTOM SUPERGLOBALS ===\n";
echo "\$_FORM: " . (isset($_FORM) ? "SET" : "NOT SET") . " - Count: " . (isset($_FORM) && is_array($_FORM) ? count($_FORM) : "N/A") . "\n";
if (isset($_FORM) && is_array($_FORM)) var_export($_FORM);
echo "\n";

echo "\$_QUERY: " . (isset($_QUERY) ? "SET" : "NOT SET") . " - Count: " . (isset($_QUERY) && is_array($_QUERY) ? count($_QUERY) : "N/A") . "\n";
if (isset($_QUERY) && is_array($_QUERY)) var_export($_QUERY);
echo "\n";

echo "\$_JSON: " . (isset($_JSON) ? "SET" : "NOT SET") . " - Count: " . (isset($_JSON) && is_array($_JSON) ? count($_JSON) : "N/A") . "\n";
if (isset($_JSON) && is_array($_JSON)) var_export($_JSON);
echo "\n\n";
